        ...
        if (ImGui::BeginPopupModal(label.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            if(validate_file)
            {
                ...
                if(check)
                {
                    ...
                    if(dialog_mode == DialogMode::SELECT && !selected_fn.empty())
                        selected_path += "/";
	                if(dialog_mode == DialogMode::SELECT && selected_fn.empty())
	                {
	                	std::string pa = std::string(selected_path.c_str(), selected_path.size()-1);

	                	std::filesystem::path p = pa;

	                	selected_fn = p.filename().string();
	                }
                    closeDialog();
                }
            }
